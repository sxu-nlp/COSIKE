...
from utils.my_loss import TwoWayLoss
import pickle
class Lite(LightningLite):
    def run(self, args, trg_train_loader, trg_valid_loader, trg_test_loader, kn_word_vectors_tensor):
    #def run(self, args, aux_train_loader, trg_train_loader, trg_valid_loader, trg_test_loader):
        ...
        def multimodal_train(self, shareSwin_model, multimodal_model, multimodal_model_optimizer, multimodal_model_scheduler, criterion):
            ...
            for i_batch, batch in enumerate(loader):
                ...
                if len(batch_HighImporFace_idx) > 0:
                    ...
                    for utt_idx in range(len(vision_inputs)):
                        for fram_idx in range(len(new_vision_mask[utt_idx])):
                            if new_vision_mask[utt_idx][fram_idx] != 0:
                                batch_vis_emo[utt_idx][fram_idx] = preds_FacialEmo_distr[
                                    batch_HighImporFace_idx[jj]]
                                new_vision_inputs[utt_idx][fram_idx] = vision_inputs[utt_idx][
                                    batch_HighImporFace_idx[jj] - margin]
                                jj += 1
                            else:
                                break
                    ...
                    del new_vision_inputs, temp_batch_HighImporFace_idx, batch_FacialEmo_import, batch_FacialEmo_import_mask, \
                        batch_HighImporFace_idx, batch_img_feat_list, batch_img_feat_temp, batch_vision_utt_feat_aux_task, preds_FacialEmo_distr

                    logits = multimodal_model(batch_text_input_ids, batch_text_input_mask, batch_text_sep_mask, audio_inputs, audio_mask, \
                                              vision_inputs, vision_mask, batch_vis_emo, batchUtt_in_dia_idx, 'train')
                ...
                truely_label_one_hot = torch.nn.functional.one_hot(batch_label_ids, 7).float().cuda()
                loss = criterion(logits, batch_label_ids.cuda()) + lambda_loss * criterion2(logits, truely_label_one_hot).cuda()
        ...
        def multimodal_evaluate(shareSwin_model, multimodal_model, criterion, test=False):
            ...
            with torch.no_grad():
                for i_batch, batch in enumerate(loader):
                    ...
                    if len(batch_HighImporFace_idx) > 0:
                        ...
                        vision_inputs_concat = torch.cat((new_vision_inputs, batch_vis_emo), dim=-1)
                        #all_vision_inputs_concat.append(vision_inputs_concat)
                        #all_vision_mask.append(new_vision_mask)
                        ...
                        logits = multimodal_model(batch_text_input_ids, batch_text_input_mask, batch_text_sep_mask,
                                                  audio_inputs, audio_mask, vision_inputs, vision_mask, batch_vis_emo, batchUtt_in_dia_idx, 'test' if test else 'val')
                        if test:
                            print(logits)
                        ...
                        '''vision_inputs_concat = torch.cat((vision_inputs, batch_vis_emo), dim=-1)
                        #all_vision_inputs_concat.append(vision_inputs_concat)
                        #all_vision_mask.append(vision_mask)
                        logits = multimodal_model(batch_text_input_ids, batch_text_input_mask, batch_text_sep_mask, audio_inputs, audio_mask, \
                                                       vision_inputs_concat, vision_mask, batchUtt_in_dia_idx)'''

                        logits = multimodal_model(batch_text_input_ids, batch_text_input_mask, batch_text_sep_mask,
                                                  audio_inputs, audio_mask, vision_inputs, vision_mask, batch_vis_emo, batchUtt_in_dia_idx)
                        if test:
                            print(logits)
            ...
            '''concatenated_vision_inputs = torch.cat(all_vision_inputs_concat, dim=0)
            all_vision_mask_inputs = torch.cat(all_vision_mask, dim=0)

            # 输出拼接后的张量的形状
            print(f"Concatenated tensor shape: {concatenated_vision_inputs.shape}")  # 应该是 (9988, 170, 519)
            print(f"Concatenated tensor shape: {all_vision_mask_inputs.shape}")

            # 创建包含两个张量的字典
            output_dict = {
                "vision_inputs_concat": concatenated_vision_inputs,
                "vision_mask_inputs": all_vision_mask_inputs
            }

            # 定义输出文件路径
            output_file = 'train_vision_inputs.pkl'

            # 将字典保存到 pkl 文件中
            with open(output_file, 'wb') as f:
                pickle.dump(output_dict, f)

            print(f"Concatenated vision_inputs_concat and vision_mask_inputs have been saved to {output_file}")'''
        ...
        '''def unimodal_evaluate(unimodal_model, criterion, test=False):
            ...
            with torch.no_grad():
                for i_batch, batch in enumerate(loader):    
                    ...
                    print(logits)
                    # 保存 logits_list 到 PKL 文件
            with open('test_logits_list.pkl', 'wb') as f:
                pickle.dump(logits_list, f)

            print("Logits 已保存到 test_logits_list.pkl 文件中。")
            avg_loss = total_loss / (args.trg_n_test if test else args.trg_n_valid)
            results = torch.cat(results)
            truths = torch.cat(truths)
            return avg_loss, results, truths'''
        ...
        if not args.doEval:
            ...
            if args.choice_modality == 'T+A+V':
                self.kn_word_vectors_tensor = kn_word_vectors_tensor
                multimodal_model = MultiModalTransformerForClassification(args, self.kn_word_vectors_tensor)
                ...
                best_share_model = torch.load(args.load_swin_path)
                ...
                share_model_optimizer = self.setup(share_model_optimizer)  # Scale your model / optimizers
            ...
            for epoch in range(1, args.num_epochs+1): 
                if args.choice_modality == 'T+A+V':
                    ...
                    multimodal_train(self, best_share_model, multimodal_model, multimodal_model_optimizer, multimodal_model_scheduler, criterion)
                    #val_loss, results, truths = multimodal_evaluate(shareSwin_model, multimodal_model, criterion, test=False)
                    val_loss, results, truths = multimodal_evaluate(best_share_model, multimodal_model, criterion, test=False)
                    end = time.time()
                    duration = end-start
        ...
        #conduct evaluation directly without training
            ...
            if args.choice_modality == 'T+A+V' :
                best_share_model = torch.load(args.load_swin_path)
                best_multi_model = torch.load(args.load_multimodal_path)
                _, results, truths = multimodal_evaluate(best_share_model, best_multi_model, criterion, test=True)
            '''elif args.choice_modality == 'V':
