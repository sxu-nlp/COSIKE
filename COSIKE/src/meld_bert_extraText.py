...
class MELD():
    ...
    def preprocess_data(self):
        ...
        if self.pretrainedBertPath.split('/')[-1] == 'roberta-large':
            ...
            tokenizer = RobertaTokenizer.from_pretrained('/home/stu4/project/meld-ccac/roberta-large/')
        elif self.pretrainedBertPath.split('/')[-1] == 'bert-large':
            print('  - Loading Bert...')
            tokenizer = BertTokenizer.from_pretrained('/home/stu4/project/meld-ccac/roberta-large/')

        features = []
        csv_path = osp.join(self.load_anno_csv, self.set_name + '_sent_emo.csv')
        int2name = make_text_dia(csv_path)  #

        emo_path = '/home/stu4/project/meld-ccac/data/dia-utt-ids/train_sent_emo.json'
        ...
        for dia_id in list(int2name.keys()):  # demo [:4]
            ...
            if self.pretrainedBertPath.split('/')[-1] == 'roberta-large':
                tokens_temp = _truncate_seq_pair(temp_utt,
                                                 Max_seq_length - 34 * 2)  # 这是计算单独的时候, 之后每个dialogue下的utterance concat一起的时候, 每个utterance会加上</s>A</s>
            elif self.pretrainedBertPath.split('/')[-1] == 'bert-large':
                tokens_temp = _truncate_seq_pair(temp_utt,
                                                 Max_seq_length - 34)  # 这是计算单独的时候, 之后每个dialogue下的utterance concat一起的时候, 每个utterance后面要加上[sep]
