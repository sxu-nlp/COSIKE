# -*- encoding: utf-8 -*-
import pickle
with open('/home/stu4/project/meld-ccac/graph/knowledge/knowedge_embedding_latter.pkl', 'rb') as f:
...
parser = argparse.ArgumentParser(description='Cross-dialogue Scene Interactive Knowledge Enhancement for Multimodal Conversation Emotion Analysis')

#---------------------------------------------------------------------------------------------------------------------------------------------#
'''MELD dataset loading'''
parser.add_argument('--load_anno_csv_path', type=str, default='/home/stu4/project/COSIKE/data/')
parser.add_argument('--meld_text_path', type=str, default='/home/stu4/project/COSIKE/data/')
parser.add_argument('--num_labels', type=int, default=7, help='classes number of meld') 
parser.add_argument('--data_load_path', type=str, default=os.path.join('/home/stu4/project/COSIKE/data/'),
                    help='path for storing the data')

#全局+局部
parser.add_argument('--global_or_local_path', type=str, default='local+global', choices='[local+global, local, global])         
...
parser.add_argument('--FacialEmoImpor_threshold', type=float, default=0.2, help='filter out the emotion-blurred frames') 
#---------------------------------------------------------------------------------------------------------------------------------------------#
#tuning
...
parser.add_argument('--weight_decay', type=float, default=0.01, help='0.01 for BERT')  
parser.add_argument('--warm_up', type=float, default=0.1, help='dynamic adjust learning rate')

parser.add_argument('--trg_batch_size', type=int, default=4, help='num of dialogues in MELD')
...
parser.add_argument('--load_unimodal_path', type=str, default='/home/stu4/project/COSIKE/preprocess_model/FacialMMT_unimodal/unimodal_model_V_06-16-01-14-14.pt',
                    help='path to load the best unimodal to evaluate on the test set')
parser.add_argument('--load_multimodal_path', type=str, default= '/home/stu4/project/COSIKE/saved_model/multimodal_model_T+A+V_08-22-18-42-09.pt',
                    help='path to load the best multimodal to evaluate on the test set')
'''parser.add_argument('--load_multimodal_path', type=str, default= '/home/stu4/project/FaicialMMT/FacialMMT-main/preprocess_model/FacialMMT-RoBERTa/multimodal_T+A+V_RoBERTa.pt',
                    help='path to load the best multimodal to evaluate on the test set')'''
parser.add_argument('--load_swin_path', type=str, default= '/home/stu4/project/COSIKE/preprocess_model/FacialMMT-RoBERTa/best_swin_RoBERTa.pt',
...
if args.choice_modality == 'V':
    ...
    args.pretrainedtextmodel_path = os.path.join('/home/stu4/project/meld-ccac/',args.plm_name)
...
trg_train_loader = DataLoader(trg_train_data, sampler=SequentialSampler(trg_train_data), batch_size=args.trg_batch_size)
...
if __name__ == '__main__':
    ...
    if args.doEval:
        ...
        #elif args.choice_modality == 'T+A+V':
        #Lite(strategy='dp', devices=1, accelerator="gpu", precision=16).run(args, None, None, None, trg_train_loader)
        Lite(strategy='dp', devices=1, accelerator="gpu", precision=16).run(args, None, None, trg_test_loader, kn_word_vectors_tensor)
    else:
        ...
        Lite(strategy='dp', devices=1, accelerator="gpu", precision=16).run(args, trg_train_loader, trg_valid_loader, trg_test_loader, kn_word_vectors_tensor)
